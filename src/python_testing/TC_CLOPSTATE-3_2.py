...
class TC_CLOPSTATE_3_2(MatterBaseTest):
    ...
    async def test_TC_CLOPSTATE_3_2(self):
        ...
        tmp_overallstate = await self.common_ops.read_overall_state_attr_with_check_of_instance()

        # STEP 4b
        self.step("4b")
        self.print_step("step number 4b", "Check field tmpOverallState.Positioning")
        asserts.assert_equal(tmp_overallstate.positioning, Clusters.ClosureOperationalState.Enums.PositioningEnum.kFullyClosed)

        # STEP 4c
        self.step("4c")
        self.print_step("step number 4c", "Check field tmpOverallState.Latching")
        asserts.assert_equal(tmp_overallstate.latching, Clusters.ClosureOperationalState.Enums.LatchingEnum.kLatchedAndSecured)

        # Start the NextStep sequence from FullyClosed
        # STEP 5a
        self.step("5a")
        self.print_step(
        ...
        # Verify the OverallState structure once motion is over
        # STEP 6a
        self.step("6a")
        ...
        asserts.assert_equal(tmp_overallstate.positioning, Clusters.ClosureOperationalState.Enums.PositioningEnum.kFullyOpened)

        # STEP 6c
        self.step("6c")
        self.print_step("step number 6c", "Check field tmpOverallState.Latching")
        asserts.assert_equal(tmp_overallstate.latching, Clusters.ClosureOperationalState.Enums.LatchingEnum.kNotLatched)

        # FollowUp NextStep sequence from FullyOpened back to FullyClosed
        # STEP 7a
        self.step("7a")
        self.print_step(
        ...
        # FollowUp NextStep sequence abortion from FullyClosed back to FullyOpened
        # STEP 9a
        self.step("9a")
        self.print_step("step number 9a", "TTH sends MoveTo command with fields Tag=SequenceNextStep(8), Speed=Low(1) to the DUT")
        cmd = Clusters.Objects.ClosureOperationalState.Commands.MoveTo(
            tag=Clusters.ClosureOperationalState.Enums.TagEnum.kSequenceNextStep,
        ...
        await self.common_ops.read_operational_state_with_check(Clusters.OperationalState.Enums.OperationalStateEnum.kRunning)
        ...
        asserts.assert_equal(tmp_overallstate.positioning, Clusters.ClosureOperationalState.Enums.PositioningEnum.kPartiallyOpened)
