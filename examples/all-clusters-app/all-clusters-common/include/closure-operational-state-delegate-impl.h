...
class ClosureOperationalStateDelegate : public Delegate
{
private:
    const Clusters::OperationalState::GenericOperationalState closureOpStateList[9] = {
    ...
    Span<const OperationalState::GenericOperationalState> mOperationalStateList =
        Span<const OperationalState::GenericOperationalState>(closureOpStateList);
    Span<const CharSpan> mOperationalPhaseList;

public:
    ClosureOperationalStateDelegate() {}
     ...
     * @param[out] get operational error after callback.
     */
    void HandleCalibrateCommandCallback(OperationalState::GenericOperationalError & err) override;

    /**
     * Handle Command Callback in application: MoveTo
     * @param[out] get operational error after callback.
     */
    void HandleMoveToCommandCallback(OperationalState::GenericOperationalError & err) override;

    /**
     * Handle Command Callback in application: ConfigureFallback
     * @param[out] get operational error after callback.
     */
    void HandleConfigureFallbackCommandCallback(OperationalState::GenericOperationalError & err) override;
};

void Shutdown();

Instance * GetClosureOperationalStateInstance();
