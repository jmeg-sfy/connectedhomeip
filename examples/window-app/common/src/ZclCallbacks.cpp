                                        ...
                                        uint8_t mask, uint16_t manufacturerCode, uint8_t type, uint16_t size, uint8_t * value)
{
    if (ZCL_WINDOW_COVERING_CLUSTER_ID == clusterId)
    {
        ...
        case ZCL_WC_TARGET_POSITION_LIFT_PERCENT100_THS_ATTRIBUTE_ID:
            WindowCovering::Attributes::GetCurrentPositionLiftPercent100ths(endpoint, &current);
            WindowCovering::Attributes::GetTargetPositionLiftPercent100ths(endpoint, &target);
            if (current > target)
            {
        ...
        case ZCL_WC_TARGET_POSITION_TILT_PERCENT100_THS_ATTRIBUTE_ID:
            WindowCovering::Attributes::GetCurrentPositionTiltPercent100ths(endpoint, &current);
            WindowCovering::Attributes::GetTargetPositionTiltPercent100ths(endpoint, &target);
            if (current > target)
            {
