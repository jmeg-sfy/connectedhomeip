...
{
    WindowApp::Instance().PostAttributeChange(endpoint, attributeId);
}
                                        ...
                                        uint16_t manufacturerCode, uint8_t type, uint16_t size, uint8_t * value)
{
    switch (clusterId)
    {
        case ZCL_WINDOW_COVERING_CLUSTER_ID:
            emberAfWindowCoveringClusterPostAttributeChangeCallback(endpoint, attributeId, mask, manufacturerCode, type, size, value);
            break;
        default:
            ChipLogProgress(Zcl, "Unknown cluster ID: %ld", clusterId);
            break;
